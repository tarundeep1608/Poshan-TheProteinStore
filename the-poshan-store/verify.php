...
<head>
  <meta charset="UTF-8">
   <title>The Poshan Store</title>
    <!-- Fontawesome and Bootstrap CDN Link -->
...
<ol class="breadcrumb" style="color: black">
    ...
    <li class="breadcrumb-item active" aria-current="page">Verify Product </li>
</ol>
<!--Site title entered-->
<section id="site-title" class="site-title">
    <div class="title-text" > Verify your Product! </div>
</section>
<!--main form-->
  <div class="form-container">
    <div class="content">
     <div class="right-side">
        <p>We place great importance on the quality and authenticity of the product. Please enter the details below.</p>
         <div class="topic-text">The 4 Digit code is mentioned on the Authentication card <i class="fas fa-certificate" style="color: #f7c142;"></i></div>
        <form action="verify.php" method="POST" id="verify-form">
            <br>
            <div>
                <p>Enter Code: </p>
                <input id="partitioned" name="code" type="text" maxlength="4" required>
            </div>
            <br>
            <div class="button">
            <input type="submit" name="submit" class="know-button" value="Verify now" >
            </div>
            <?php
            if(isset($_POST['submit']))
            {
            $conn = mysqli_connect("localhost", "root", "", "Poshan") or die("Connection Error: " . mysqli_error($conn));
            $code = $_POST['code'];
            $result = mysqli_query($conn, "SELECT productcode FROM verify where productcode = '$code' ");
//             $result = mysqli_query($conn, "SELECT productcode FROM verify WHERE productcode = '$code'");
            if(mysqli_num_rows($result) > 0)
                {
                        echo '<script language="javascript">';
                        echo 'alert("Yayyy! Your product is 100% safe and legit!")';
                        echo '</script>';
            ...
            {
                       echo '<script language="javascript">';
                       echo 'alert("Your submitted product code was not found. Please check and submit again")';
                       echo '</script>';
